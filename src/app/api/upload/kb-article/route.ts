...
export async function POST(request: Request) {
  ...
  if (!file || !articleId) {
    return NextResponse.json({ error: 'Arquivo e article_id são obrigatórios' }, { status: 400 })
  }
  ...
  const path = `${articleId}/${Date.now()}_${safeFilename}`
  const buffer = await file.arrayBuffer()

  const serviceSupabase = await createServiceClient()
  const { error: uploadError } = await serviceSupabase.storage
    .from('kb-article-attachments')
    .upload(path, buffer, { contentType: file.type, upsert: false })

  if (uploadError) return NextResponse.json({ error: uploadError.message }, { status: 500 })

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { error: dbError } = await serviceSupabase.from('kb_article_attachments').insert({
